...
export async function GET(request: NextRequest) {
  ...
  const statsSQL = `SELECT
    ...
    COALESCE(SUM(CASE WHEN capex > 0 AND status NOT IN ('won','completed','rejected') THEN capex ELSE 0 END), 0) as totalCapex
  ...
  let totalMonthly = 0;
  try {
    // Exclude won/completed (live clients, counted in /api/clients/stats) and rejected
    // (no longer a real prospect — would inflate pipeline forecast)
    let monthlySQL = `SELECT COALESCE(SUM(CAST(cfv.value AS REAL)), 0) as v
      FROM custom_field_values cfv JOIN leads l ON cfv.lead_id = l.id
      WHERE cfv.field_id = 'custom_monthly' AND cfv.value != ''
        AND l.status NOT IN ('won','completed','rejected')`;
