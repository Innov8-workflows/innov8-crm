...
type ViewId = "prospects" | "projects" | "clients" | "dashboard" | "map" | "pricing" | "referrals";
  ...
  return (
    <div className="flex items-center gap-2 px-4 py-2 flex-shrink-0" style={{ background: "var(--nav-bg)", borderBottom: "2px solid var(--surface2)" }}>
      ...
      <button
        ...
        onClick={() => {
          const themes = ["dark", "light", "midnight", "slate", "forest", "purple", "pink"] as const;
          const labels: Record<string, string> = { dark: "Dark", light: "Light", midnight: "Midnight", slate: "Slate", forest: "Forest", purple: "Purple", pink: "Pink" };
        ...
        title="Cycle theme: Dark → Light → Midnight → Slate → Forest → Purple → Pink"
      >
        ...
        <span id="theme-label" className="hidden sm:inline">
          {typeof window !== "undefined" ? ({ dark: "Dark", light: "Light", midnight: "Midnight", slate: "Slate", forest: "Forest", purple: "Purple", pink: "Pink" }[localStorage.getItem("crm_theme") || "dark"] || "Dark") : "Dark"}
        </span>
      </button>
    </div>
  );
