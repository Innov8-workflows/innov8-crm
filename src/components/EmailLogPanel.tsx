  ...
  return (
    <div className="fixed inset-y-0 right-0 w-full sm:w-[420px] shadow-2xl z-50 flex flex-col"
        ...
        <div className="grid grid-cols-2 gap-2 text-sm">
          ...
          {lead.demo_site_url && (
            <div className="col-span-2"><span style={{ color: "var(--text-dim)" }}>Demo:</span>{" "}
              <a href={lead.demo_site_url} target="_blank" rel="noreferrer" style={{ color: "var(--accent)" }} className="hover:underline text-xs">{lead.demo_site_url}</a>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        {loading ? <p className="text-sm" style={{ color: "var(--text-tertiary)" }}>Loading...</p> : activeSection === "timeline" ? (
          <>
            {!showActivityForm ? (
              <button className="w-full mb-3 px-3 py-2 text-sm rounded-lg flex items-center gap-2 transition-colors"
                style={{ color: "var(--accent)", border: "1px solid rgba(234,88,12,0.2)", background: "rgba(234,88,12,0.05)" }}
              ...
              <div className="mb-3 p-3 rounded-lg space-y-2" style={{ border: "1px solid rgba(234,88,12,0.2)", background: "rgba(234,88,12,0.05)" }}>
                <div className="flex gap-1 flex-wrap">
                  {ACTIVITY_TYPES.map((t) => (
                    <button key={t.value} className="px-2 py-1 text-xs rounded-full transition-colors"
                      style={{
                        background: activityType === t.value ? "var(--accent)" : "var(--surface2)",
                        color: activityType === t.value ? "#fff" : "var(--text-muted)",
                ...
                <div className="flex gap-2">
                  <button onClick={addActivity} className="px-3 py-1 text-xs rounded font-medium" style={{ background: "var(--accent)", color: "#fff" }}>Save</button>
                  <button onClick={() => setShowActivityForm(false)} className="px-3 py-1 text-xs" style={{ color: "var(--text-dim)" }}>Cancel</button>
                </div>
              </div>
            )}
            {timeline.length === 0 ? (
              <p className="text-sm text-center py-4" style={{ color: "var(--text-quaternary)" }}>No activity yet</p>
            ) : (
                ...
                {timeline.map((item) => (
                  <div key={item.id} className="flex gap-3 py-2" style={{ borderBottom: "1px solid var(--surface2)" }}>
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs mt-0.5" style={{ color: "var(--text-quaternary)" }}>{fmt(item.date)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <>
            <div className="mb-3 flex gap-2">
              ...
              <button onClick={addNote} className="px-3 py-1.5 text-xs rounded font-medium" style={{ background: "var(--accent)", color: "#fff" }}>Add</button>
            </div>
            {notes.length === 0 ? (
              <p className="text-sm text-center py-4" style={{ color: "var(--text-quaternary)" }}>No notes yet</p>
            ) : (
              <div className="space-y-2">
                {notes.map((note) => (
                  <div key={note.id} className="p-3 rounded-lg" style={{ background: "var(--surface2)", border: "1px solid var(--border)" }}>
                    <p className="text-sm whitespace-pre-wrap" style={{ color: "#ddd" }}>{note.content}</p>
                    <p className="text-xs mt-1" style={{ color: "var(--text-tertiary)" }}>{fmt(note.created_at)}</p>
                  </div>
                ))}
              </div>
            )}
